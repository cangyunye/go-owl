...
### 2026-09-18：智谱 ZCode 静默上传用户完整 Git 历史

昨日（2026-09-18）安全社区曝出智谱官方 AI 编程桌面端 **ZCode** 存在严重隐私问题（[扒一扒 ZCode 静默上传全量 Git 历史的骚操作](https://blog.ferstar.org/posts/zcode-silent-workspace-snapshot-upload/)）：

- 登录状态下，客户端会在后台**静默打包整个工作区（含完整 `.git` 历史、LFS 缓存、reflog、全局应用配置）**，加密后直传阿里云 OSS，全程无需用户操作；
- 采用信封加密，**RSA 公钥由服务端动态下发、私钥仅存云端**——本地密文连用户自己和客户端都无法解密，只有智谱后端可解；
- 快照内容近九成是 `.git` 数据（86.6%），历史上被覆盖删除的敏感配置、未推送的分支名、内部 GitLab 域名等全部随包上传；
- 设置中的「优化体验」「仓库快照索引」开关**均无法关闭该上传**，只要登录即常开，且删除本地快照会自动重新打包上传；
- **未经用户同意**：整个采集与上传过程无任何授权弹窗、知情提示或选择入口，而智谱官网却公开承诺"保护用户隐私"，言行严重相悖；
- 隐私政策、官方文档对该行为**只字未提**，官方"优化计划默认关闭、不主动加入不会用于训练"的声明与事实不符。

**防御建议**：在文件系统层锁定快照目录 `~/.zcode/v2/checkpoints`（macOS：`chflags uchg`，Linux：`chattr +i`），或直接停用 ZCode。
