...
「剧本管理」页（`Alt+4`，operator 及以上可见）左侧为剧本分类，主区为剧本列表（名称/分类/描述/任务），每条提供 Run、二次编辑、下载、删除；下方为运行历史与运行详情：
...
顶栏下方是标签栏：每个标签承载「一级导航 + 侧栏子菜单 + 详情」整条上下文，切标签不丢筛选。

- **点侧栏导航**：复用已打开的同视图标签（保住里面的分组/搜索/翻页）；**Ctrl/⌘+点击**或**中键**才新开标签，
  因此可以同时开两个「节点管理」标签，各自停留在不同分组
- **标签内下钻**：列表 → 详情（节点详情、任务详情、终端、SFTP）在原标签内进行；点导航图标可回到该视图列表
- **筛选进 URL**：节点页的 `?group=&q=&status=`、剧本页的 `?cat=&q=` 会写进地址栏并出现在标签标题上
  （如「节点管理 · web · "db"」），所以这类上下文可收藏、可分享、刷新不丢
- **中键点列表行**：在新标签打开该行详情（节点详情 / 任务详情）
- **标签操作**：`×` 或中键关闭；右键菜单支持关闭其他/右侧/全部；支持拖拽排序；最多 8 个标签
- 标签集合与激活项会记住，刷新后自动恢复
...
| `Alt+T` | 新建标签（上限 8 个） |
| `Alt+W` | 关闭当前标签 |
| `Alt+PageUp` / `Alt+PageDown` | 上一个 / 下一个标签 |
