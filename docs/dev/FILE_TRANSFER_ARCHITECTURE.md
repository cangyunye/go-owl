...
`owl file transfer` 实现了节点间扩散传输功能。核心思想是将目标节点构造成一棵 **K 叉扩散树**，按层级分批发起传输，实现有节奏的批量分发。
...
> **源节点定义**：在扩散树中有子节点的节点即为源节点。此例中仅 Node 1 是源节点。
...
| 要点 | 说明 |
|------|------|
| **数据始终从控制节点发出** | 当前实现不是真正的节点到节点 P2P 接力。扩散树仅用于编排**传输顺序**——决定哪些节点先收、哪些后收。每一层的文件仍然由控制节点直连目标节点进行传输。 |
| **源节点是逻辑角色** | 源节点由树结构自动决定（有子节点即为源），用户无法直接指定。源节点本身只是"先收到文件的节点"，不负责实际转发。 |
| **成功才扩散** | 只有状态为 `Completed` 的节点，其子节点才会进入下一批传输队列。失败的源节点会导致其子树被跳过（代码中有失败重分配逻辑 `ReassignFailedNode`）。 |
| **分批的意义** | 避免对大量节点同时建立 SSH 连接造成的资源压力，实现有节奏的分批扩散。 |
| **源节点无额外校验** | 扩散树构建和传输状态管理中，不对源节点做 SSH 可达性、磁盘空间、带宽等检查。所有节点的连接信息在调用链上游通过 `NodeResolver.ResolveMultiple()` 统一解析。 |

### 6.5 调用链

```
runTransfer()                          // cmd/cli/cmd/file/transfer.go
  ├─ TreeBuilder.Build(targets)        // 构建扩散树
  │    └─ DiffusionTree.GetSourceNodes()
  ├─ DiffusionTransfer.InitializeStatuses()
  ├─ BFS 层序遍历扩散树
  │    └─ TransferManager.Upload()     // 每层并行上传
  │         ├─ scp / rsync 探测
  │         └─ 返回每节点 TransferResult
  ├─ DiffusionTransfer.UpdateNodeStatus()  // 标记成功/失败
  └─ history.RecordOperation()             // 记录历史
```
