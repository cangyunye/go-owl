...
`PUT /settings/:key` 对部分内置键做了值校验（非法值返回 400）：

| key | 校验规则 |
|-----|----------|
| `staging_dir` | 必须为绝对路径 |
| `staging_min_free` | 必须为正整数（GB） |

### 文件中转站配置

"文件传输"页的**文件中转站**（暂存区）通过两个设置项控制，可在**系统设置**页直接修改（admin）：
...
- 系统设置页的配置表格包含 **Description 列**，对内置键显示中文说明
- 尚未写入数据库的配置项显示默认值并带 **(默认值)** 标记，编辑保存后写入数据库
- 编辑弹窗内 `staging_min_free` 会校验正整数（GB）

修改后立即生效：文件传输页中转站的文件路径与磁盘信息（`GET /staging/disk`）会反映新的暂存地址。
